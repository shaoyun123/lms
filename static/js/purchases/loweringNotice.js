...
layui.use(['admin', 'form', 'table', 'layer', 'laydate', 'formSelects'], function () {
  ...
  var loweringNoticeName = {
    ...
    data: function (data) {//需要提交给后台的对象
      var obj = {
        'buyerIdList': data.loweringNotice_buyerId,
        'integratorIdList': data.loweringNotice_integratorId,
        'developerIdList': data.loweringNotice_developerId,
        ...
        switch (dealType) {
            case '1':
                return [ //表头
                    ...
                    , { field: 'thirtySalesNum', title: '30天销量',width: 80 }
                    , { field: 'originPurPrice', title: '采购成本(￥)' }
    ...
    initPersonByType: function (type) {
      ...
      switch (type) {
        case 1:
          _this.initAjax(1, 'loweringNotice_buyerId') //采购人员
          break
        case 2:
          _this.initAjax(2, 'loweringNotice_integratorId') //整合人员
          break
        case 3:
          _this.initAjax(3, 'loweringNotice_developerId') //开发人员
          break
      }
    },
    initAjax: function (val, ele) {
      $.ajax({
        ...
        success: function (res) {
          if (res.code = '0000') {
            var data = res.data
            appendSelect($('#' + ele), data, 'id', 'userName')
            layui.formSelects.render(ele)
          }
        },
        error: function (err) {
          layer.msg('请求错误')
  ...
  // 模板查询赋值
   commonSaveSearchTpl({
    id: "loweringNotice_save",
    formId: "loweringNotice_form",
    pageName: "loweringNotice",
    searchBtnId: "loweringNotice_submit",
    cb: param => loweringNotice_formVal(param),
    btnText: '',
    layerTitleText: ''
  })

  function loweringNotice_formVal(param) {
    $('#loweringNotice_reset').trigger('click')
    // 页签
    if (param.loweringNotice_dealType) {
      $(".numCount_loweringNotice")
      .each(function () {
          let liIndex = $(this).data("code")
          if (liIndex == param.loweringNotice_dealType) {
    ...
    // 给表单赋值
    form.val("loweringNotice_form", param)
    form.render()
    let multiSelectObj = {
      loweringNotice_buyerId: "loweringNotice_buyerId",
      loweringNotice_integratorId: 'loweringNotice_integratorId',
      loweringNotice_developerId: 'loweringNotice_developerId'
    };
    ...
    // 执行搜索
    $(".numCount_loweringNotice.layui-this").click()
  }

  //监听iframe传参
  window.addEventListener('message', function(event){
    console.log('iframe传来的数据是', event.data)
    let { isSale, ifManager } = event.data
    if (isSale || ifManager) {
      $('#loweringNotice_form').find('[name=loweringNotice_isSale]').val(isSale)
      if (ifManager == 'false') {
        var username = localStorage.getItem('lmsAppUserName');
        // 获取采购专员下拉框数据
        let purcahseUser = $('#loweringNotice_form [name=loweringNotice_buyerId] option')
