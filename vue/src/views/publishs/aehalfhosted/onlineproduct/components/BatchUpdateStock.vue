  ...
  >
    ...
    <div>
      ...
      >
        ...
        <vxe-column field="storeSSku" title="店铺子SKU" />
...
<script setup>
  ...
  const checkTotal = computed(() =>
    props.activeKey === 1
      ...
      : tableData.value.filter((item) => item.isCheckbox).length
  );
  ...
  const getBatchGetStockList = async (type) => {
    clearTableChecked();
    // 初始查询 type为0
    if (type === 1 && form.queryContent === '') {
      return ElMessage.warning('请输入SKU！');
    }

    if (form.queryContent !== '') {
      form.skus = form.queryContent.split(',');
    }
    const productIdsObj = type ? {} : { productIds: pIdList.value };
    // 已加入/可预约tab添加参数
    const tabObj =
      props.activeKey === 3
        ? {}
    ...
    const { data, extra } = await getBatchGetStockApi({
      ...form,
      ...productIdsObj,
      ...tabObj
    }).finally(() => {
  ...
  const BatchWriteFn = () => {
    ...
    if (props.activeKey === 4) {
      checkedList = tableData.value.filter((item) => item.isCheckbox);
    }
    if (!checkedList.length) return ElMessage.warning('请选择数据');
    checkedList.forEach((item) => {
      // 原值
      let originVal = item.oldStock;
  ...
  const handleSave = async () => {
    ...
    const newStockNullList = checkedList.filter((item) => !item.newStock);
  ...
  const getOperateResult = async (batchNo) => {
    ...
    Object.keys(data).forEach((key) => {
      const index = tableData.value.findIndex((el) => el.variationId === key);
      if (index > -1) {
