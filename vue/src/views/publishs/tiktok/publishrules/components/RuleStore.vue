        ...
        <vxe-grid ref="tableRef" v-bind="gridOptions">
          ...
          <template #toolbar_default="{ row }">
            <div class="toolbar-btn">
              ...
              <div v-permission="['shopee_store_list_edit']">
                <el-button
                  type="primary"
...
<script setup>
        ...
        { required: true, message: '请输入每天刊登量' },
        {
          validator({ cellValue }) {
            return new Promise((resolve, reject) => {
              setTimeout(() => {
                if (!inRange(cellValue, 1, 241)) {
                  reject(new Error('每天可刊登量最小为1，最大为240'));
                } else {
                  resolve();
                }
              }, 100);
  ...
  const handleSaveCurDailyPublishNums = async (row) => {
    if (!inRange(row.dailyPublishNums, 1, 241))
      return ElMessage.warning('每天刊登量最小为1，最大为240');
    try {
      tableLoading.value = true;
