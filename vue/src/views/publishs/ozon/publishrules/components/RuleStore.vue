        ...
        <vxe-grid ref="tableRef" v-bind="gridOptions">
          <template #daily_publish_nums_edit="{ row }">
            <el-input-number
              ...
              max="500"
              :step="1"
              @change="handleSaveCurDailyPublishNums(row)"
            ></el-input-number>
          </template>
          <template #toolbar_default="{ row }">
            <div class="toolbar-btn">
              ...
              <div v-permission="['ozon_store_list_edit']">
                <el-button
                  type="primary"
...
<script setup>
        ...
        { required: true, message: '请输入每天刊登量' },
        {
          validator({ cellValue }) {
            return new Promise((resolve, reject) => {
              setTimeout(() => {
                if (!inRange(cellValue, 1, 501)) {
                  reject(new Error('每天可刊登量最小为1，最大为500'));
                } else {
                  resolve();
                }
              }, 100);
  ...
  const handleBatchRemove = async () => {
    ...
    try {
      console.log(idList);
      const { msg } = await deleteRuleShopApi(idList.join(','));
      needFresh.value = true;
      // 删除行
  ...
  const handleSaveCurDailyPublishNums = async (row) => {
    if (!inRange(row.dailyPublishNums, 1, 501))
      return ElMessage.warning('每天刊登量最小为1，最大为500');
    try {
      tableLoading.value = true;
