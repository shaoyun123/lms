	...
	$.fn.scrollFix = function(options) {
		return this.each(function() {
			...
			function onScroll() {
				...
				var ScrollTop = $('.layadmin-tabsbody-item.layui-show').scrollTop();
				if (opts.bottom != -1) {
					// ScrollTop = ScrollTop + $(window).height() - outerHeight - opts.bottom;
					ScrollTop = ScrollTop + $('.layadmin-tabsbody-item.layui-show').height() - outerHeight - opts.bottom;
				}
				// console.log('滚动距离', ScrollTop)
				// console.log('结束定位',endfix)&& (ScrollTop < endfix)
				if (ScrollTop > toTop) {
					if (ie6) { //IE6则使用这个样式
