...
(function(layui, window, factory) {
            ...
            this.config = {
                ...
                filter: (id, inputVal, val, isDisabled) => {
                    if (inputVal.indexOf(',') > 0) {
                        let inputValArr = inputVal.trim().split(',');
                        // return !inputValArr.includes(val.name);
                        const result = inputValArr.filter(item => val.name.includes(item));
        ...
        if (searchUrl) {
            ...
            if (!ajaxConfig.beforeSearch || (ajaxConfig.beforeSearch && ajaxConfig.beforeSearch instanceof Function && ajaxConfig.beforeSearch(id, searchUrl, inputValue))) {
                ...
                fs.clearid = setTimeout(() => {
                    ...
                    reElem.find(`dd.${FORM_NONE}`).addClass(FORM_EMPTY).text('请输入要搜索的内容');
    ...
    Common.prototype.renderData = function(id, dataArr, linkage, linkageWidth, isSearch, isReplace) {
        ...
        let reElem = $(`.${PNAME} dl[xid="${id}"]`).parents(`.${FORM_SELECT}`);
        let originalSelects = reElem.parents(`div[fs_id="${id}"]`).siblings('select')
        let ajaxConfig = ajaxs[id] ? ajaxs[id] : ajax;
    ...
    Common.prototype.createDD = function(id, item, clz) {
        ...
        return `<dd lay-value="${item.value}" class="${item.disabled ? DISABLED : ''} ${clz ? clz : ''}" ${attr}>
					<div class="xm-unselect xm-form-checkbox ${item.disabled ? DISABLED : ''}"  style="margin-left: ${(pid.length - 1) * 20}px">
    ...
    Common.prototype.selectAll = function(id, isOn, skipDis) {
        ...
        dl.find(`dd[lay-value]:not(.${FORM_SELECT_TIPS}):not(.${THIS}):not(.${DD_HIDE})${skipDis ? ':not(.'+DISABLED+')' :''}`).each((index, item) => {
            item = $(item);
    ...
    Select4.prototype.btns = function(id, btns, config) {
        ...
        $.each(target, (key, val) => {
            ...
            if (btns.length) {
                ...
                let html = common.renderBtns(key, show, config && config.space ? config.space : '20px');
                dd.html(html);
            } else {
