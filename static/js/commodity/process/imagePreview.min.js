 ...
 * modified by ztt on 2019/12/05
 */
var ImagePreview = {
    default_params:{},
    init:function (param) {
        if (param) {
            $.extend(this.default_params, param); //如果参数存在,就把参数添加到default_params里
        }
        ip_global_img_list = [];
        var id = this.default_params.id; //获取参数的id
        if (typeof id == 'string') {
    ...
    fnBindEvent:function (ip_global_img_list) {
        ...
        //给每一个子元素绑定事件(弹出事件)
        this.onClickEvent(ip_global_img_list[ImagePreview.default_params.index]);
        // $(ip_global_img_list).on("click", this.onClickEvent);
        //绑定拖拽事件(id是插入的大图imgId)
    ...
    onClickEvent:function (img) {
        //ImagePreview是整个对象,这里这样用是因为this指向问题
        ImagePreview.fnReset();
        $("#ip-img-preview").attr("src", $(img).attr("src"));//$(this)表示点击的图片
        ImagePreview.fnAdjustMaxWidth(); //长度判定
        ip_global_cur = ImagePreview.fnGetIndexOfCurImg($(img));
        //遮罩层出现,动画效果fadeIn
