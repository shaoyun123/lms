<template>
  <el-dialog
    :model-value="singleDialogVisible"
    title="可配单品"
    width="90%"
  ...
  >
    ...
    <div>
      ...
      >
        ...
        <vxe-column field="prodSSku" title="商品子SKU"></vxe-column>
        ...
        <vxe-column field="restNumber" title="剩余可用" width="100">
          <template #default="{ row }">
            <span>{{
              row.whStock
...
<script setup>
  ...
  const searchDataBtn = async () => {
    ...
    let unSaveGood = await queryPackableSingle(params).finally(
      () => (singleLoading.value = false)
    );

    tablePage.total = unSaveGood.data.length;
    nums.value = `数量:${unSaveGood.data.length}`;
    singleData.value = unSaveGood.data;
    tableData.value = singleData.value.slice(0, tablePage.pageSize);
  ...
  const getData = async () => {
    try {
      ...
      delete params.platCode;
      let unSaveGood = await queryPackableSingle(params);
      singleLoading.value = false;
      tablePage.total = unSaveGood.data.length;
      nums.value = `数量:${unSaveGood.data.length}`;
      singleData.value = unSaveGood.data;
  ...
  const handlePageChange = ({ currentPage, pageSize }) => {
    ...
    tableData.value = singleData.value.slice(startItem, endItem);
    updateTotalQuantity();
    batchSkuNum.value = tableData.value.filter((item) => item.batchNo)?.length;
