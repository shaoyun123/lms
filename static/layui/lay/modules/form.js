                ...
                select: function() {
                        ...
                        , events = function(reElem, disabled, isSearch) {
                            ...
                            var showDown = function() {
                                    /**
                                     * $win.height(): 浏览器窗口的高度,不包括工具栏
                                     * top: 选中dd距离dl顶部的高度
                                     * dlHeight: 下拉框dd的高度
                                    */
                                    var top = reElem.offset().top + reElem.outerHeight() + 5 - $win.scrollTop(),
                                        dlHeight = dl.outerHeight(); //下拉框的高度
                                    index = select[0].selectedIndex; //获取最新的 selectedIndex
                                    reElem.addClass(CLASS + 'ed');
                                    dds.removeClass(HIDE);
                                    nearElem = null;
                                    //初始选中样式
                                    var DLheight = dds.eq(index).height() * index; //获取到元素所在位置的高度
                                    if ( DLheight > dlHeight) {
                                        dl.scrollTop(DLheight-dlHeight/2);
                                        if (top + dlHeight > $win.height() && top >= dlHeight) {
                                            reElem.addClass(CLASS + 'up'); //控制弹出的下拉框方向: 上或者下
                                        }
                                        return false;
                                    }
                                    ...
                                    if(reElem.parents(".layui-inline").data("up")) {
                            ...
                            input.on('keyup', function(e) { //键盘松开
                                ...
                                // var firstEle = reElem.find('.layui-anim.layui-anim-upbit>dd:not(.layui-hide):first');
                                // firstEle.addClass(THIS).siblings().removeClass(THIS);
                                //Tab键展开
                                if (keyCode === 9) {
                                    showDown();
                                }
                            ...
                            var notOption = function(value, callback, origin) {
                                ...
                                layui.each(dds, function() {
                                    var othis = $(this),
                                        text = othis.text(),
                                        not = text.toLowerCase().indexOf(value.toLowerCase()) === -1;
                            ...
                            if (isSearch) {
                                input.on('keyup', search).on('blur', function(e) {
                                    ...
                                    initValue = $(select[0].options[selectedIndex]).text();
                                    setTimeout(function() {
                                        notOption(input.val(), function(none) {
