      ...
      <div v-for="(elem, key) of searchList" :key="key">
        ...
        >
          ...
          <div class="main-content">
            ...
            <div v-if="item.buyerAvatar" class="buyer-img">
              <el-image
                class="image"
                fit="contain"
            ...
            <div class="info-content">
              <div class="info-content-title flex-between">
                <div class="title">{{ item.buyerNick }}</div>
                <div>
              ...
              <div v-if="item.platOrderId" class="last-msg">
                [订单]<span class="match-color">{{ item.platOrderId }}</span>
              </div>
...
<script setup>
  ...
  import {
    readConversationItemApi,
    createANewConversationApi
  } from '@/api/tiktok/chat';
  import useTikTokChatStore from '@/store/modules/tiktokChat';
  import { storeToRefs } from 'pinia';
  const tikTokChatStore = useTikTokChatStore();
  const { activeSearchIndex } = storeToRefs(tikTokChatStore);
  const { fetchMessage } = tikTokChatStore;
  ...
  const cardTypePopover = 'searchListCard';
  ...
  const onClick = async (item, key, index) => {
    ...
    if (item.convShortId) {
      ...
      const { data } = await createANewConversationApi(item);
      item.convShortId = data.convShortId;
      fetchMessage(item);
    }
  };
</script>
