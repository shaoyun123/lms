...
layui.define(['jquery'], function (exports) {
    ...
    if ($('#ew-css-selectInput').length <= 0) {
        layui.link('/lms/static/style/selectInput.css');
    }
    // 加载全局鼠标位置
    ...
    var selectInput = function (opt) {
        ...
        this.options = $.extend(true, {
            ...
            focusShow: opt.focusShow || false,
            focusMethod: opt.focusMethod || null
        }, opt);
        this.init();  // 初始化
    ...
    selectInput.prototype.bindEvents = function () {
        ...
        if(that.options.focusShow){
            components.$inputElem.off('focus').on('focus', function () {
                // 执行我们的正常的操作
                var value = $(this).val();
                // 执行重载dl数据
                var focusCb = function (data) {
                    if (data && typeof (data) === 'string') {
                        data = JSON.parse(data)
                    }
                        if (!data) data = [];
                ...
                if(!value){
                    that.options.focusMethod(value,focusCb);
                }
            })
        }
