  ...
  <div class="">
    ...
    >
      ...
      >
        ...
        <el-form-item label="每日加入数量" prop="dailyPublishNums">
          <el-input-number
            ...
            :step="1"
          />
          <span class="ml10">大于0,小于240</span>
        </el-form-item>

        <el-form-item label="加入开始时间" prop="publishTime">
        ...
        <el-form-item label="加入间隔时间" prop="publishInterval">
          ...
          <!-- <span class="ml10">加入量 x 间隔时间 必须小于 1200</span> -->
        </el-form-item>
      </el-form>
...
<script setup>
  ...
  const validateDailyPublishNums = (rule, value, callback) => {
    ...
    if (value === undefined || value === null) {
      callback(new Error('请输入每天加入量'));
    } else if (dailyPublishNums > 240 || dailyPublishNums < 0) {
      callback(new Error('每天可加入量最小为1,最大为240'));
      // } else if (publishInterval * value >= 1200) {
      //   callback(new Error('加入量 x 间隔时间 必须小于 1200'));
    } else {
      callback();
    }
  };
