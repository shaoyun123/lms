<template>
  ...
  <div class="publishrules_wrapper app-container" :loading="pageLoading">
      ...
      <div class="page-header">
        <div style="display: flex; justify-content: end; margin-right: 30px">
          <el-dropdown class="ml12">
            ...
            <template #dropdown>
              <el-dropdown-menu>
                <div v-permission="['ae_rules_bacthOnRule']">
                  <el-dropdown-item @click="handleBatchStatus(1)"
                    >批量开启</el-dropdown-item
                  >
                </div>
                <div v-permission="['ae_rules_bacthOffRule']">
          ...
          <div v-permission="['ae_rules_list_edit']">
            <el-button
              class="ml12"
              type="primary"
    ...
    />
    <PublishRulesLog
      v-if="aePublishRulesLogDialog"
      :show-dialog="aePublishRulesLogDialog"
      :log-table-data="aeLogTableData"
      @close-dialog="handleLogRuleClose($event)"
    />
  </div>
</template>

<script setup name="publishsaehalfhostedautojoin">
  ...
  onMounted(async () => {
    ...
    Promise.all([
      getStoreList('AE' + encodeURIComponent('半托管')),
      manageListApi()
    ])
      .then((res) => {
  ...
  const handleDetail = (row, operationType) => {
    ...
    if (operationType !== 'new') {
      ...
      if (row?.listingTimeEnd || row?.listingTimeStart) {
        time = [row.listingTimeStart, row.listingTimeEnd];
      }
  ...
  // 日志弹窗--start
  const aePublishRulesLogDialog = ref(false);
  const aeLogTableData = ref();
  const handleLogRule = async (row) => {
    try {
      let params = {
        ruleId: row.id,
        platCode: 'AE' + encodeURIComponent('半托管')
      };
      const { data } = await getRuleModifyLogs(params);
      aeLogTableData.value = data.map((item) => ({
        ...item,
        ruleName: row.ruleName
      }));
      aePublishRulesLogDialog.value = true;
    } catch (err) {
      console.log('err :>> ', err);
    }
  };
  const handleLogRuleClose = (e) => {
    aeLogTableData.value = [];
    aePublishRulesLogDialog.value = e.isShow;
  };
</script>
