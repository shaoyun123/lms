...
<script setup>
  ...
  const init = () => {
    ...
    for (let key in style) {
      if (formData.value?.[key]) {
        formData.value[key].value = JSON.parse(JSON.stringify(style[key]));
      }
    }
    // 字体大小
    ...
    if (formData.value?.text) {
      formData.value.text.value = $(dom).find('.detail').text();
    }
    //  邮编默认大写
  ...
  const changeStyle = (key, val) => {
    ...
    if (key === 'text') {
      ...
      //   // 邮编默认大写
      //   curDom.find('span').each(function () {
      //     const spanDom = $(this);
      //     if (spanDom.data('text') === 'postcode') {
      //       spanDom.css(key, val);
      //     }
      //   });
      ...
      curDom.css(key, val);
      if (val === 'normal') {
        curDom.css('overflow', 'unset');
      } else {
        curDom.css('overflow', 'hidden');
      }
    } else {
      curDom.css(key, val);
    }
  ...
  const handleReset = () => {
    ...
    for (let key in formData.value) {
      ...
      if (key === 'text') {
        //文本
        $(dom).find('.detail').text(formData.value.text.value);
      } else if (key === 'lineHeight') {
        // 行距
