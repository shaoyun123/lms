<template>
  <div class="z-select" :style="{ height: !open ? 'unset' : '350px' }">
    <!-- 点击输入框部分[存放选中的值] -->
    <div ref="zselect" class="z-select__wrapper" @click="open = !open">
      <div class="z-select__container">
        ...
        <el-input readonly placeholder="请选择">
          <template #suffix>
...
<script setup name="zSelect">
  ...
  // const zCheckedList = ref(props.modelValue); //选中的值
  ...
  const setDropdownPosition = () => {
    const { width } = zselect.value.getBoundingClientRect();
    // zdropdown.value.style.top = `${top + 35}px`;
    // zdropdown.value.style.left = `${left}px`;
    zdropdown.value.style.width = `${width}px`;
  };
  // 监听下拉框的显示隐藏
  watch(open, (newVal) => {
    if (newVal) {
  ...
  const zCheckedClear = ref(false); //是否清空
