        ...
        let vm = new Vue({
            ...
            data () {
                return {
                    ...
                    basicHeaderCols: [
                        { prop: "price", label: "零售价（USD）", width: "200", isShow: true },
                        { prop: "priceCny", label: "零售价（CNY）", width: "200", isShow: true },
                        { prop: "ipmSkuStock", label: "商家仓库存", width: "300", isShow: true },
                        { prop: "storeSubSku", label: "商品编码", width: "200", isShow: true },
                    ],// 基础属性的tableheader
            ...
            mounted () {
                ...
                commonReturnPromise({
                    ...
                    message: err,
                    type: 'error',
                    customClass: 'smtModifySskuInfo-message',
                    offset: 150
                }))
            ...
            methods: {
                fetchData (objData) {
                    // 选中数据  多维转一维
                    var _chooseData = objData.syncSSmtSkuDtoList.map(item => (item.aliexpressSkuPropetry||{}).aeopSkuProperty).flat(Infinity)
                    if (
                    ...
                    var deliveryPlaceList = chooseData.filter(item => item.skuPropertyId == 200007763)
                    if (deliveryPlaceList.length) {
                        this.plaPriceData = deliveryPlaceList.map(item =>
                            ({ deliveryPlace: item.propertyValueDefinitionName, tpl: '', propertyValueId: item.propertyValueId })
                        )
                    }

                    //数据整合
                    var data = objData.cateAttrInfoList.map(item => {
                        var chooseList = chooseData.filter(elem => elem.skuPropertyId == item.id)
                            .map(_item => {
                                return item.customizedName && item.customizedPic ? { ..._item, checked: true, skuImage: _item.skuImage || '' }
                                    : { ..._item, checked: true }
                            })
                        item.customizedName && item.customizedPic && chooseList.push({ checked: false, skuImage: '', skuPropertyId: item.id, })   //多添加一行
                    ...
                    this.initData = data.map(item => {
                        if (item.chooseList.length) {
                            return {
                                ...item,
                                platCateAttrValueVOList: this.matchData(item.chooseList, item.platCateAttrValueVOList)
                            }
                        } else {
                            return {
                                ...item,
                                platCateAttrValueVOList: item.platCateAttrValueVOList.map(elem => ({ ...elem, checked: false, propertyValueDefinitionName: elem.attrValue }))
                            }
                        }
                    })
                    ...
                    this.syncSSmtSkuDtoList = objData.syncSSmtSkuDtoList.map(item=>{
                        if(item.aliexpressSkuPropetry && item.aliexpressSkuPropetry && item.aliexpressSkuPropetry.aeopSkuProperty.length){
                            for(let i = 0; i < item.aliexpressSkuPropetry.aeopSkuProperty.length; i++){
                                let curPropertyObj = item.aliexpressSkuPropetry.aeopSkuProperty[i]
                                if(!curPropertyObj.propertyValueDefinitionName){
                                   ( chooseAttrListObj[curPropertyObj.skuPropertyId].platCateAttrValueVOList||[]).forEach(elem=>{
                                    if(elem.id===curPropertyObj.propertyValueId){
                                        curPropertyObj.propertyValueDefinitionName=elem.attrValue
                                    }
                                   })
                                }
                            }
                    ...
                    let hasSkuIdObj = {}
                    var skuDtoList = (objData.syncSSmtSkuDtoList[0].aliexpressSkuPropetry||{}).aeopSkuProperty || []
                    ...
                    // 结合
                    this.syncSSmtSkuDtoListCols = headerCols.concat(this.basicHeaderCols)
                },
                // 输入框的checkbox
                inputChkHandle (isChecked, obj, curChkObj) {
                    var deliveryPlaces = obj.platCateAttrValueVOList.filter(item => !!item.checked)
                ...
                inputAttrNameHandle (val, curObj, curSkuObj) {
                    ...
                    if (curObj.checked) {
                        ...
                        var _val = val || curSkuObj.platCateAttrValueVOList.filter(item => {
                            if (item.id == (curObj.propertyValueId || curObj.id)) {
                                curObj.id && (curObj.propertyValueDefinitionName = item.attrValue)
                                return true
                            }
                        })[0].attrValue
                        this.syncSSmtSkuDtoList = this.tableChangeRowSomeSku(curObj, _val, 'propertyValueDefinitionName')
                    }
                },
                // 获取调价
                getRegionPrice (data) {
                ...
                selPicHandle (showbox, chooseObj, index, curObj) {
                    ...
                    if (!showbox && chooseObj.propertyValueId) {
                        ...
                        for (var i = 0; i < curObj.platCateAttrValueVOList.length; i++) {
                            if (curObj.platCateAttrValueVOList[i].id == chooseObj.propertyValueId) {
                                curObj.platCateAttrValueVOList[i].checked = true
                            }
                        }
                        var propertyValueDefinitionName = curObj.platCateAttrValueVOList.filter(item => item.id === chooseObj.propertyValueId)[0].attrValue
                        // 最后一项选择后，选择数据，增加一行,并且选中
                        if (index == curObj.chooseList.length - 1 && !!chooseObj.propertyValueId) {
                            curObj.chooseList.push({ checked: false, skuImage: '', skuPropertyId: curObj.id })
                            chooseObj.checked = true
                            // 添加行数据
                            var curskuDtoObj = {
                                propertyValueDefinitionName: chooseObj.propertyValueDefinitionName || propertyValueDefinitionName,
                            ...
                            if (curObj.chooseList.length == 2) {
                                ...
                                this.syncSSmtSkuDtoListCols = this.syncSSmtSkuDtoListCols.
                                    map(item => ({ ...item, isShow: curObj.id == item.id ? true : item.isShow }))
                                this.syncSSmtSkuDtoList.length ? this.syncSSmtSkuDtoList.forEach(item => {
                                    ...
                                    price: '', ipmSkuStock: '', storeSubSku: "",priceCny:'',
                                    aliexpressSkuPropetry: { aeopSkuProperty: [{ ...curskuDtoObj }] }
                                })
                            } else {
                                // 已有一行
                                // 删除本属性
                                var chooseOtherSkuList = this.syncSSmtSkuDtoList.map(item => {
                                    var _item = item.aliexpressSkuPropetry.aeopSkuProperty.filter(elem => elem.skuPropertyId != curObj.id)
                                    return _item
                                })
                                // 需要去重   有问题????
                                var reduceObj = []

                                var _chooseOtherSkuList = chooseOtherSkuList[0].length ? chooseOtherSkuList.reduce((cur, next) => {
                                    let isExit = reduceObj.filter(elem => next.every(item => !!elem[item.propertyValueId]))
                                    if (!isExit.length) {
                                        let _reduceObj = {}
                                        next.forEach(item => {
                                ...
                                _chooseOtherSkuList.forEach(item => {
                                    this.syncSSmtSkuDtoList.push({
                                        ...
                                        aliexpressSkuPropetry: {
                                            aeopSkuProperty: item ? [curskuDtoObj].concat(item) : [curskuDtoObj]
                                        }
                                    })
                                })
                            }
                        } else { //修改table行数据
                            var _propertyValueDefinitionName = chooseObj.propertyValueDefinitionName || propertyValueDefinitionName
                            this.syncSSmtSkuDtoList = this.tableChangeRowSomeSku({ propertyValueId: this.lastSelectedValId, propertyValueDefinitionName: _propertyValueDefinitionName }, chooseObj.propertyValueId, 'propertyValueId')
                        }
                ...
                //上传图片那的下拉框  没选中，删除当前行,table减少相关行（如果是当前属性唯一一个，删除一列）
                checkOptionHandle (isChecked, curSkuObj, curObj) {
                    if (!isChecked) {
                        // 删除列
                        if (curSkuObj.chooseList.length == 2) {
                            this.syncSSmtSkuDtoListCols = this.syncSSmtSkuDtoListCols.
                                map(item => ({ ...item, isShow: curObj.skuPropertyId == item.id ? false : item.isShow }))
                            this.syncSSmtSkuDtoList = this.syncSSmtSkuDtoList.map(item => {
                                var itemSku = item.aliexpressSkuPropetry.aeopSkuProperty.filter(elem => elem.propertyValueId != curObj.propertyValueId)
                                return {
                                    ...item,
                            ...
                            // table 减少相关行
                            this.syncSSmtSkuDtoList = this.syncSSmtSkuDtoList.filter(item => {
                                var ittem = true
                                let itemArr = item.aliexpressSkuPropetry.aeopSkuProperty
                                for (var i = 0; i < itemArr.length; i++) {
                                    if (itemArr[i].propertyValueId == curObj.propertyValueId) {
                                        return ittem = false
                                    }
                                }
                                return ittem
                        ...
                        // 减少行
                        curSkuObj.chooseList = curSkuObj.chooseList.filter(item => item.propertyValueId != curObj.propertyValueId)
                        // 将option的这个值 的checked为false
                        curSkuObj.platCateAttrValueVOList = curSkuObj.platCateAttrValueVOList.map(item => ({
                            ...item,
                            checked: item.id === curObj.propertyValueId ? false : item.checked
                        }))
                    }
                },
                // 删除图片
                delImg (obj) {
                    obj.skuImage = ''
                    this.syncSSmtSkuDtoList = this.tableChangeRowSomeSku(obj, '', 'skuImage')
                },
                // 添加图片
                uploadImg (event, obj) {
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            ...
                            if (data.code == '0000') {
                                ...
                                // 修改table 行
                                if (obj.checked) {
                                    _this.syncSSmtSkuDtoList = _this.tableChangeRowSomeSku(obj, data.msg, 'skuImage')
                                }
                            }
                        },
                    ...
                    if (obj.id == 200007763) {
                        if (isChecked) {
                            this.plaPriceData[0].deliveryPlace ?
                                this.plaPriceData.push({ deliveryPlace: curChkObj.attrValue, tpl: '', propertyValueId: curChkObj.id })
                                : this.plaPriceData = [{ deliveryPlace: curChkObj.attrValue, tpl: this.plaPriceData[0].tpl, propertyValueId: curChkObj.id }]
                        } else {
                            let _plaPriceData = this.plaPriceData.filter(item => item.propertyValueId != curChkObj.id)
                            this.plaPriceData = _plaPriceData.length ? _plaPriceData : [{ deliveryPlace: '', tpl: "", propertyValueId: '' }]
                        }
                    }
                    this.tableChangeRow(isChecked, obj, curChkObj, deliveryPlaces)
                },
                ...
                tableChangeRowSomeSku (obj, curval, curName) {
                    var tableData = this.syncSSmtSkuDtoList.map(item => {
                        var _item = item.aliexpressSkuPropetry.aeopSkuProperty.map(elem => {
                            if (elem.propertyValueId == (obj.id || obj.propertyValueId)) {  //因为图片的输入框与checkbox输入框的循环体不一样
                                return { ...elem, propertyValueDefinitionName: obj.propertyValueDefinitionName || item.propertyValueDefinitionName, [curName]: curval }
                            } else {
                                return { ...elem }
                            }
                        })
                ...
                tableChangeRow (isChecked, obj, curChkObj, checkedList) {
                    ...
                    if (!checkedList.length) {
                        this.syncSSmtSkuDtoListCols = this.syncSSmtSkuDtoListCols.
                            map(item => ({ ...item, isShow: obj.id == item.id ? false : item.isShow }))
                        this.syncSSmtSkuDtoList.forEach(item => {
                            var skuDtoindex = item.aliexpressSkuPropetry.aeopSkuProperty.findIndex(elem => elem.skuPropertyId == obj.id)
                            skuDtoindex != -1 && item.aliexpressSkuPropetry.aeopSkuProperty.splice(skuDtoindex, 1)
                        })
                    } else if (isChecked && checkedList.length == 1) { // 如果是 从无变成有 增加一列
                        // 增加一列
                        this.syncSSmtSkuDtoListCols = this.syncSSmtSkuDtoListCols.
                            map(item => ({ ...item, isShow: obj.id == item.id ? true : item.isShow }))
                        ...
                        if (!isChecked) {
                            this.syncSSmtSkuDtoList = this.syncSSmtSkuDtoList.filter(item => {
                                let isIncluCurSku = item.aliexpressSkuPropetry.aeopSkuProperty
                                    .filter(elem => elem.propertyValueId == curChkObj.id)
                            ...
                            // 删除本属性
                            var chooseOtherSkuList = this.syncSSmtSkuDtoList.map(item => {
                                var _item = item.aliexpressSkuPropetry.aeopSkuProperty.filter(elem => elem.skuPropertyId != obj.id)
                                return _item
                            })
                            // 需要去重   写的太复杂了
                            var reduceObj = []
                    ...
                    this.$nextTick(() => {
                        this.$refs['smtModifySskuInfo_table'].doLayout()
                    })
                },
                // 选择调价后，调价模板数
                selregionPriceHandle (type) {
                    this.regionPriceList = this.allRegionPriceList.filter(item => item.adjustPriceType == type)
                ...
                estimatedPrice () {
                    ...
                    if (!hasSubSkuVal) return this.$message({
                        message: '商品编码不能有空值',
                        type: 'warning',
                        customClass: 'smtModifySskuInfo-message',
                        offset: 100
                    });
                    if (hasRepeatSubSku) return this.$message({
                        message: '商品编码不能有重复的',
                        type: 'warning',
                        customClass: 'smtModifySskuInfo-message',
                        offset: 100
                ...
                priceHandle (rowValue) {
                    if(rowValue.price==='' || rowValue.price === undefined){
                        ...
                        rowValue.priceCny = (rowValue.price * this.exchange_rate).toFixed(2)
                    }
                },
                // 
                priceCnyHandle(rowValue){
                    if(rowValue.priceCny === '' || rowValue.priceCny === undefined){
                        ...
                        rowValue.price = (rowValue.priceCny / this.exchange_rate).toFixed(2)
                    }
                },
                // 库存
                ...
                tableOtherPicRow (curRowObj, id) {
                    var aeopSkuPropertys = curRowObj.aliexpressSkuPropetry.aeopSkuProperty
                    var curAeopSkuPropertys = aeopSkuPropertys.filter(item => item.skuPropertyId == id)
                    return `<div class="smtModifySskuInfo-disfcenter" 
                    v-if="${!!(curAeopSkuPropertys[0] || {}).skuImage}">
                    <img class="img_show_hide lazy w40"
                        original="${(curAeopSkuPropertys[0] || {}).skuImage || ''}"
                        src="${(curAeopSkuPropertys[0] || {}).skuImage || ''}"
                        style="display: block;">
                </div>
                <div class="taCenter ml10">${(curAeopSkuPropertys[0] || {}).propertyValueDefinitionName || ''}</div>`
                },
                ...
                verfityRepeatNullSku () {
                    ...
                    for (var i = 0; i < this.syncSSmtSkuDtoList.length; i++) {
                        if (this.syncSSmtSkuDtoList[i].storeSubSku && storeSubSkuObj[this.syncSSmtSkuDtoList[i].storeSubSku]) {
                ...
                skuInfoSubmit () {
                    ...
                    let submitInfo = {
                        ...
                        skuList: this.syncSSmtSkuDtoList.map(item => ({
                            ...
                            aeopSkuPropertyList: item.aliexpressSkuPropetry?this.removeChineseCharacters(item.aliexpressSkuPropetry.aeopSkuProperty) : []
                        }))
                    }
                ...
                removeChineseCharacters(aeopSkuProperty){
                    let _aeopSkuProperty =(aeopSkuProperty||[]).map(item=>({
                        ...item,
                        propertyValueDefinitionName: (/[\u4e00-\u9fa5]/).test(item.propertyValueDefinitionName) ? '' : item.propertyValueDefinitionName
                    }))
                ...
                submitPriceAdjustPrice () {
                    ...
                    if (this.isRegionPrice) {
                        ...
                        if (!hasTemplateId) return this.$message({
                            message: '调价模板需要选值',
                            type: 'warning',
                            customClass: 'smtModifySskuInfo-message',
                            offset: 150
                        ...
                        let skuRegionAdjustPrice = {
                            id: smt_arr[0].id,
                            deliveryList: this.plaPriceData.map(item => ({ deliveryPlace: item.propertyValueId || 'NONE', templateId: item.tpl }))
                        }
                        commonReturnPromise({
                ...
                showAddSkuArea(curAttr) {
                    const isExistOther = this.syncSSmtSkuDtoListShowCols.filter(
                    (item) => item.id && item.id !== curAttr.id
                    ).length;

                    if (!isExistOther || this.syncSSmtSkuDtoListShowCols.length === 5) {
                ...
                addSkuAndImgattr(newSkuList, curAttr) {
                    ...
                    if (curPropertyValueId) {
                        ...
                        this.syncSSmtSkuDtoListShowCols.length === 4
                            ...
                            this.syncSSmtSkuDtoList[0].ipmSkuStock= item.ipmSkuStock || 999
                        } else {
                        this.syncSSmtSkuDtoList.push({
                            price: "",
