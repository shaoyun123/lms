...
<script setup>
  ...
  import {
    // updateShopRef,
    getBindingStoresByRuleId,
    getAllStoresByRuleIdAndStoreName
  ...
  //数据回显,已选中的店铺
  getBindingStoresByRuleId(ruleId.value).then((res) => {
    if (res.code === '0000' && res.data) {
      selectedData.value = res.data;
      selectedIdsArr.value = res.data.map((item) => item.id);
  ...
  const saveHandle = async () => {
    ...
    //   };
    //   const { code } = await updateShopRef(params);
    // if (code === '0000') {
    ...
    axios({
      url: '/api/chat/lazada/autoReplyRule/updateShopRef',
      method: 'POST',
      params: {
        ruleId: ruleId.value,
        newStoreAcctIdList: selectedData.value.map((item) => item.id).join(','),
      ...
      .then((res) => {
        if (res.data.code === '0000') {
          ...
          errStore.value = res.data.data.map((item) => item.storeAcctId);
