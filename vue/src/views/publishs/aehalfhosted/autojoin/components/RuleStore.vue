      ...
      <el-card v-loading="tableLoading">
        <template #header>
          <div class="store_width">
            <span class="publish-num-label">店铺</span>
            <el-select
              v-model="storeName"
              collapse-tags
              multiple
              filterable
              clearable
              :filter-method="filterStoreName"
            >
              <el-option
                v-for="item in storeNameOtions"
                :key="item.value"
                :value="item.value"
                :label="item.label"
              />
            ...
            <div style="margin-left: 20px">
              自动加入规则：{{ checkedRow.ruleName }}
            </div>
          </div>
          <div class="card-header">
            <span class="ml10">总数：{{ totalCount }}</span>
            <div>
              <span class="publish-num-label">每日加入量</span>
              <el-input-number
                ...
                :max="240"
              />
              <span class="publish-num-label ml10">加入开始时间</span>
              ...
              <span class="publish-num-label ml10">加入间隔时间</span>
        ...
        <vxe-grid ref="tableRef" v-bind="gridOptions">
          <template #daily_publish_nums_edit="{ row }">
            <el-input-number
              ...
              :max="240"
              :step="1"
              @focus="handleSaveCurDailyPublishNumsFocus(row)"
          ...
          <template #toolbar_default="{ row }">
            <div class="toolbar-btn">
              ...
              <div v-permission="['ae_store_list_edit']">
                <el-button
                  type="primary"
...
<script setup>
  ...
  import {
    ...
    editRuleShopApi,
    deleteRuleShopApi
  } from '@/api/publishs/aeautojoin';
  import StoreInfoDialog from './StoreInfoDialog.vue';
  import { ElMessage } from 'element-plus';
  import { cloneDeep, debounce, inRange } from 'lodash-es';
  ...
  const gridOptions = reactive({
    ...
    editRules: {
      dailyPublishNums: [
        { required: true, message: '请输入每天加入量' },
        {
          validator({ cellValue }) {
            return new Promise((resolve, reject) => {
              setTimeout(() => {
                if (!inRange(cellValue, 1, 240)) {
                  reject(new Error('每天可加入量最小为1,最大为240'));
                } else {
                  resolve();
                }
              }, 100);
  ...
  // 支持多个筛选
  const filterStoreName = (keyword) => {
    if (keyword) {
      const _keyword = keyword.split(',');
      storeNameOtions.value = storeNameList.value.filter(
        (item) => _keyword.filter((v) => item.label.includes(v)).length
      );
    } else {
      storeNameOtions.value = cloneDeep(storeNameList.value);
    }
  };
  // 对table数据的店铺名列进行筛选
  watch(
  ...
  const handleApplyPublishNums = async () => {
    ...
    //     unableStoreList.join() + ': 加入量 x 间隔时间 必须小于 1200'
    //   );
    // }
  ...
  const dailyPublishNums = ref('');
  const handleSaveCurDailyPublishNumsFocus = (row) => {
    dailyPublishNums.value = row.dailyPublishNums;
  };
  const handleSaveCurDailyPublishNums = async (row) => {
    if (!inRange(row.dailyPublishNums, 1, 240)) {
      row.dailyPublishNums = dailyPublishNums.value;
      return ElMessage.warning('每天加入量最小为1,最大为240');
    }
    try {
      tableLoading.value = true;
