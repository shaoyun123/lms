  ...
  <div class="publishrules_wrapper app-container" :loading="pageLoading">
    ...
    <el-card v-loading="tableDataLoading" class="list_card">
      <div class="page-header">
        <el-dropdown
          v-permission="[
            'tiktok_rules_bacthOnRule',
            'tiktok_rules_bacthOffRule'
          ]"
          class="ml12"
        >
      ...
      <vxe-grid ref="tableRef" v-bind="gridOptions">
        ...
        <template #toolbar_default="{ row }">
          <div class="flex-wrap">
            ...
            <!-- <div v-permission="['tiktok_rules_addCopyBtn']">
            ...
            <div>
              <el-button type="primary" @click="handleOperationLog(row)"
                >执行日志</el-button
              >
            </div>
          </div>
    ...
    <OperationLog v-model="tiktokOperationLogDialog" :log-info="logInfo" />
  </div>
</template>

<script setup name="publishstiktokpublishrules">
  ...
  onMounted(async () => {
    ...
    Promise.all([
      ...
      .then((res) => {
        ...
        initList.value.tiktokCateList = res[2]?.data || [];
        getCreatorList();
      })
      .catch((err) => {
        console.log('err :>> ', err);
  ...
  // 操作日志
  const tiktokOperationLogDialog = ref(false);
  const logInfo = ref({});
  const handleOperationLog = async (row) => {
    logInfo.value = {
      id: row.id,
      ruleName: row.ruleName
    };
    tiktokOperationLogDialog.value = true;
  };
