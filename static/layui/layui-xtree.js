...
layuiXtree.prototype.Initial = function (o) {
    ...
    _this._isCheckOnly = o.isCheckOnly != null ? o.isCheckOnly : false;  // 只选中自己，by maji
    _this._isParentChangeCheck = o.isParentChangeCheck != null ? o.isParentChangeCheck : true;  // 子节点选中是否改变父节点，by maji
...
layuiXtree.prototype.dataBind = function (d) {
    ...
    if (d.length > 0) {
        for (var i = 0; i < d.length; i++) {
            ...
            _this._domStr += '<input type="checkbox"  class="layui-xtree-checkbox" ' + xtree_isend + xtree_ischecked + xtree_isdisabled + ' value="' + d[i].value + '" title="' + d[i].title + '" remark="' + d[i].remark + '" lay-skin="primary" lay-filter="xtreeck' + _this._containerid + '">';
...
layuiXtree.prototype.Rendering = function () {
    ...
    if (_this._ckall) {
        _this._form.on('checkbox(xtreeckall' + _this._containerid + ')', function (data) {
            ...
            _this._ckallSuccess(data); //增加全选回调数据data以此来获取全选的选中状态  by maji
        });
    }
}
...
//重载方法，更新渲染，data为新的url， by maji
layuiXtree.prototype.renderByData = function (data) {
    var _this = this;
...
layuiXtree.prototype.ParendCheck = function (ckelem) {
    ...
    if (_this._isParentChangeCheck){
        ...
        if (xtree_p.getAttribute('class') == 'layui-xtree-item') {
            ...
            if(xtree_all.length == xtree_count){
                //子节点全部选中时，才触发上一级父节点改变
                _this.getChildByClassName(xtree_p, 'layui-xtree-checkbox')[0].checked = true;
                _this.getChildByClassName(xtree_p, 'layui-xtree-checkbox')[0].nextSibling.classList.add('layui-form-checked');
            } else {
                _this.getChildByClassName(xtree_p, 'layui-xtree-checkbox')[0].checked = false;
...
//获取全部选中的checkbox对象，授予资源用 by maji
//授予平台，则调用GetChecked
...
//清除其他多选框选中效果  by maji
layuiXtree.prototype.SetOtherCheckedFalse = function (id) {
    var _this = this;
    var cks = _this.getByClassName('layui-xtree-checkbox');
    for (var i = 0; i < cks.length; i++) {
        if (cks[i].checked && cks[i].value != id) {
            cks[i].checked = false;
            cks[i].nextSibling.classList.remove('layui-form-checked');
        }
    }
}

layuiXtree.prototype.setCheckedByValue = function (value) {
    var _this = this;
    var cks = _this.getByClassName('layui-xtree-checkbox');
    for (var i = 0; i < cks.length; i++) {
        if (cks[i].value == value) {
            cks[i].checked = true;
            cks[i].nextSibling.classList.add('layui-form-checked');
        }
    }
}

layuiXtree.prototype.setMultiCheckedByValue = function (value) {
    var _this = this;
    var valsArr = value.split(',');
    var cks = _this.getByClassName('layui-xtree-checkbox');
    for (var i = 0; i < cks.length; i++) {
        if (valsArr.includes(cks[i].value)) {
            cks[i].checked = true;
            cks[i].nextSibling.classList.add('layui-form-checked');
        }
    }
...
//获取选中的checkbox的val为空字符的对象
//适用于授予平台账号业务，全选一个平台则自动授权   by maji
layuiXtree.prototype.GetCheckedNullValue = function () {
    var _this = this;
    var arr = new Array();
    var arrIndex = 0;
    var cks = _this.getByClassName('layui-xtree-checkbox');
    for (var i = 0; i < cks.length; i++) {
        if (cks[i].checked && cks[i].getAttribute('value') == "") {
            arr[arrIndex] = cks[i]; arrIndex++;
        }
    }
    return arr;
...
//获取全部选中的checkbox对象  包括非末级checkbox
layuiXtree.prototype.GetCheckedIncludeParent = function () {
    var _this = this;
    var arr = new Array();
    var arrIndex = 0;
    var cks = _this.getByClassName('layui-xtree-checkbox');
...
// 根据末级 value。 展开这1分支
layuiXtree.prototype.OpenByLeafValue = function (a) {
    var _this = this;
    var xtreeIcon = []

    function getParentIconByValue(value) {
        var parentCheck = _this.GetParent(value)
        if (parentCheck != null) {
            var pItem = $(parentCheck).closest('.layui-xtree-item')
            xtreeIcon.push(pItem.children('.layui-xtree-icon'))
            getParentIconByValue(parentCheck.value)
        }
    }
    getParentIconByValue(a)
    for(var i = xtreeIcon.length -1; i >=0; i--){
        if (xtreeIcon[i].attr('data-xtree') == '0') {
            xtreeIcon[i].click()
        }
    }
}

// 根据title, 进行搜索，并展开所有符合的分支,并标记红色
layuiXtree.prototype.searchLikeByTitle = function (searchTitle) {
    var _this = this;
    var arr = new Array();
    var arrIndex = 0;
    var cks = _this.getByClassName('layui-xtree-checkbox');
    ...
    var showCheck
    for (var i = 0; i < cks.length; i++) {
        showCheck = $(cks[i]).siblings('.layui-form-checkbox')
        // 清除标红
        showCheck.css('background-color','')
        if (cks[i].title.indexOf(searchTitle) >= 0 ) {
            matchList.push(cks[i].value)
            // 标红
            showCheck.css('background-color','pink')
        }
    }
    console.log(matchList)
    for (var j = 0; j < matchList.length; ++j) {
        _this.OpenByLeafValue(matchList[j])
    }
}