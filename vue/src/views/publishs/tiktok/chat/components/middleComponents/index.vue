<template>
  <div class="tikTokChat">
    <header v-if="userInfo?.convShortId" class="chat_header">
      <!-- 买家头像 -->
      <el-avatar
        v-if="userInfo.buyerAvatar"
        :size="50"
        :src="userInfo.buyerAvatar"
        class="md_headpic"
      />
      <!-- 买家姓名，当前所在店铺 -->
      <div class="chat_header_info">
        <span>{{ userInfo.buyerNick }}</span>
        <template v-if="userInfo.storeAcct">
          <div class="chat_header_store">
            <el-icon :color="chatPrimary"><Shop /></el-icon>
            <span>{{ userInfo.storeAcct }}</span>
          </div>
        </template>
      </div>
      <!-- 聊天对话框 收藏取消店铺 -->
      <div class="chat_header_pinned">
        <div v-if="userInfo.pinned" @click="handlePinned(false)">
          <el-icon :size="28" class="collect_star"><StarFilled /></el-icon>
        </div>
        <div v-else @click="handlePinned(true)">
          <el-icon :size="28"><Star /></el-icon>
        </div>
      </div>
    </header>
    <section ref="chatMainRef" v-loading="messageListLoading" class="chat_main">
      <ChatList
        @scan-img-func="scanImgFunc"
        @change-offer-count="changeOfferCount"
        @change-offer-status="changeOfferStatus"
      />
      <!-- 点击图片查看大图 -->
      <div v-if="largeImg" class="lzd-preview">
        <div class="lzd-preview__header">
          <a
            class="action-btn action-btn--download"
            download=""
            :href="largeImgUrl"
            target="_blank"
            ><el-icon style="font-size: 40px"><Download /></el-icon></a
          ><span class="action-btn action-btn--close" @click="largeImg = false"
            ><el-icon style="font-size: 40px"><Close /></el-icon
          ></span>
        </div>
        <div class="lzd-preview__content">
          <div
            class="lzd-preview__viewer"
            :style="{ 'background-image': 'url(' + largeImgUrl + ')' }"
            style="transform: scale(1)"
          ></div>
        </div>
      </div>
      <!-- 上传图片回显 -->
      <div v-if="dialogVisible" class="lzd-preview">
        <div class="lzd-preview__content">
          <div
            class="lzd-preview__viewer"
            :style="{ 'background-image': 'url(' + dialogImageUrl + ')' }"
            style="transform: scale(1)"
          ></div>
        </div>
        <div class="lzd-preview__footer">
          <el-button
            size="small"
            type="danger"
            @click="handleImgFileSend(uploadImgUrl)"
            >发送</el-button
          >
          <el-button
            size="small"
            type="danger"
            @click="dialogVisible = !dialogVisible"
            >取消</el-button
          >
        </div>
      </div>
    </section>
    <!-- 底部 -->
    <div class="chat-footer">
      <!-- 选择栏 -->
      <el-col :span="24">
        <el-row class="flex-center">
          <!-- 表情和图片选择 -->
          <el-col :span="2">
            <!-- <el-col :span="4"> -->
            <el-row>
              <el-col :span="4"></el-col>
              <el-col :span="10">
                <el-upload
                  class="upload_img"
                  :action="''"
                  :auto-upload="false"
                  :show-file-list="false"
                  :on-change="handlePictureCardPreview"
                >
                  <el-icon :size="30"><Picture /></el-icon>
                </el-upload>
              </el-col>
            </el-row>
          </el-col>
          <!-- 语言模板 -->
          <el-col :span="10" class="flex-center">
            <el-select
              v-model="tplInfo.languageCode"
              filterable
              placeholder="语言"
              @change="handleGetTplNameList"
            >
              <el-option
                v-for="item in languageList"
                :key="item"
                :label="item"
                :value="item"
              />
            </el-select>

            <el-select
              v-model="tplInfo.templateTypeName"
              filterable
              placeholder="模板类型"
              @change="handleGetTplNameList"
            >
              <el-option
                v-for="item in tplTypeList"
                :key="item"
                :label="item"
                :value="item"
              />
            </el-select>

            <el-select
              v-model="tplId"
              filterable
              placeholder="模板名称"
              @change="handleTplName"
            >
              <el-option
                v-for="item in tplNameList"
                :key="item.id"
                :label="item.name"
                :value="item.id"
              />
            </el-select>
          </el-col>
        </el-row>
      </el-col>
      <!-- 输入消息内容框 -->
      <el-col :span="24" class="relative">
        <el-input
          v-model="replyTextarea"
          type="textarea"
          :rows="5"
          placeholder="在此输入消息内容"
          autofocus="true"
          resize="none"
        >
        </el-input>
        <div class="flex-center transform-box">
          <el-select
            v-model="languageForm.selectedLanguage"
            style="width: 100px"
            placeholder="语言"
          >
            <el-option
              v-for="item in languageList"
              :key="item"
              :label="item"
              :value="item"
            />
          </el-select>
          <el-button
            size="small"
            :loading="transLoading"
            type="danger"
            @click="handleTextTranslate"
            >翻译</el-button
          >
        </div>
        <el-input
          v-model="textarea"
          type="textarea"
          :rows="3"
          placeholder="翻译为选定语言"
          autofocus="true"
          resize="none"
          @keydown="handleKeyCode($event)"
        >
        </el-input>
        <el-button
          class="reply-box"
          size="small"
          type="danger"
          @click="handleTextSend"
          >回复</el-button
        >
      </el-col>
      <!-- 回复按钮 -->
      <!-- <el-col :span="24">
        <el-row type="flex" class="row-bg mtb8" justify="end">
          <el-button size="small" type="danger" @click="handleTextSend"
            >回复</el-button
          >
        </el-row>
      </el-col> -->
    </div>
  </div>
</template>

<script setup>
  import {
    onMounted,
    ref,
    nextTick,
    watch,
    onBeforeUpdate,
    reactive
  } from 'vue';
  import { Shop, Picture, Download, Close } from '@element-plus/icons-vue';
  import useTikTokChatStore from '@/store/modules/tiktokChat';
  import ChatList from './ChatList.vue';
  import { storeToRefs } from 'pinia';
  import { comBlobToDataURL } from '@/utils/upload';
  import { ElMessage } from 'element-plus';
  import { isEmpty } from 'lodash-es';
  import { chatPrimary } from '@/styles/vars.module.scss';
  import {
    getLanguageBySiteApi,
    getTranslateFromBaiduOrGoogleApi
  } from '@/api/tiktok/chat';
  import {
    getTplTypeListApi,
    getTplNameListApi,
    getTplContentApi
  } from '@/api/common/chat';

  const tikTokChatStore = useTikTokChatStore();
  const { sendMessage, handlePinned } = tikTokChatStore;
  const { userInfo, messageListLoading } = storeToRefs(tikTokChatStore);
  const changeOfferCount = () => {};
  const changeOfferStatus = () => {};

  const languageForm = reactive({
    selectedLanguage: ''
  });

  onMounted(() => {
    document.addEventListener('paste', getClipboardFiles);
  });

  const dialogVisible = ref(false);
  const dialogImageUrl = ref();
  // 发送图片
  const handleImgFileSend = (imageUrl) => {
    if (isEmpty(userInfo.value)) {
      return ElMessage.warning('请选择聊天对象');
    }
    sendMessage({ messageType: 'file_image', params: { imageUrl } });
    dialogVisible.value = false;
  };

  // 截图之后 cv粘贴上传图片
  const getClipboardFiles = (event) => {
    let items = event.clipboardData && event.clipboardData.items;
    let file = null;
    if (items && items.length) {
      // 检索剪切板items
      for (var i = 0; i < items.length; i++) {
        if (items[i].type.indexOf('image') !== -1) {
          file = items[i].getAsFile();
        }
      }
    }
    if (!file) {
      return false;
    }

    dialogImageUrl.value = URL.createObjectURL(file); // 赋值图片的url，用于图片回显功能
    dialogVisible.value = true;
    uploadImg(file); // 调用上传文件api接口
  };
  const uploadImgUrl = ref();
  // 图片上传服务器
  const uploadImg = (file) => {
    // 文件上传操作
    comBlobToDataURL(file, (res) => {
      console.log('res :>> ', res);
      uploadImgUrl.value = res;
    });
  };

  watch(
    () => userInfo.value.salesSite,
    (val) => {
      if (val) {
        getLanguageList();
        replyTextarea.value = '';
        textarea.value = '';
        initTplInfo();
      }
    }
  );

  // 翻译 语言必选校验
  const transLoading = ref(false);
  const firstTransContent = ref('');
  const replyTextarea = ref('');
  const handleTextTranslate = async () => {
    if (!languageForm.selectedLanguage) {
      return ElMessage.warning('请先选择语言!');
    }
    if (!replyTextarea.value) {
      return ElMessage.warning('不能翻译空白消息!');
    }

    transLoading.value = true;
    const { code, msg } = await getTranslateFromBaiduOrGoogleApi({
      toLanguage: languageForm.selectedLanguage,
      query: replyTextarea.value
    }).finally(() => {
      transLoading.value = false;
    });
    if (code === '0000') {
      textarea.value = msg;
      firstTransContent.value = msg;
    } else {
      ElMessage.warning(msg);
    }
  };

  // 键盘回车事件
  const textarea = ref();
  const handleKeyCode = (event) => {
    // 排除别的键盘事件，如果不加这个，按一下键盘就会触发一次
    if (event.keyCode === 13) {
      if (event.keyCode === 13 && event.ctrlKey === true) {
        textarea.value = textarea.value + '\n';
      } else {
        event.preventDefault();
        handleTextSend();
      }
    }
  };
  // 发送文字消息
  const textParams = ref({});
  const handleTextSend = () => {
    if (isEmpty(userInfo.value)) {
      return ElMessage.warning('请选择聊天对象');
    }
    if (!replyTextarea.value && !textarea.value) {
      return ElMessage.warning('不能发送空白消息！');
    }

    textParams.value.text = replyTextarea.value;
    if (textarea.value) textParams.value.text = textarea.value;
    textParams.value.traslateContent = '';
    if (
      replyTextarea.value &&
      firstTransContent.value &&
      firstTransContent.value === textarea.value
    ) {
      textParams.value.traslateContent = replyTextarea.value;
    }

    const itemIdReg = /^[0-9]{11}$/;
    const orderReg = /^[0-9A-Z]{14}$/;
    if (itemIdReg.test(textarea.value)) {
      // itemId
      sendMessage({
        messageType: 'goods_card',
        params: { goodsId: textarea.value }
      });
    } else if (orderReg.test(textarea.value)) {
      sendMessage({
        messageType: 'order_card',
        params: { orderId: textarea.value }
      });
    } else {
      sendMessage({
        messageType: 'text',
        params: textParams.value
      });
    }
    textarea.value = '';
    replyTextarea.value = '';
  };

  // 选择图片回显
  const handlePictureCardPreview = (file) => {
    const supportedTypes = [
      'image/jpg',
      'image/jpeg',
      'image/png',
      'image/webp',
      'image/gif'
    ];
    const isSupported = supportedTypes.includes(file.raw.type);

    // const isLt2M = file.raw.size / 1024 / 1024 < 0.5  //限制上传文件的大小
    if (!isSupported) {
      return ElMessage.warning('上传图片只能是 JPG/JPEG/PNG/WEBP/GIF 格式!');
    } else {
      dialogImageUrl.value = URL.createObjectURL(file.raw); // 赋值图片的url，用于图片回显功能
      dialogVisible.value = true;
      uploadImg(file.raw); // 调用上传文件api接口
    }
  };

  // 点击查看大图
  const largeImg = ref(false);
  const largeImgUrl = ref();
  const scanImgFunc = (imgSrc) => {
    largeImg.value = true;
    largeImgUrl.value = imgSrc;
  };

  // 滚动条在最底部
  const chatMainRef = ref();
  const scrollToBottom = () => {
    nextTick(() => {
      chatMainRef.value.scrollTop = chatMainRef.value.scrollHeight;
    });
  };

  // 获取语言
  const languageList = ref([]);
  const getLanguageList = async () => {
    const { data } = await getLanguageBySiteApi({
      salesSite: userInfo.value.salesSite
    });
    languageList.value = data;
    languageForm.selectedLanguage = data[0];
    tplInfo.languageCode = data[0];
  };

  // #region 语言模板
  const tplInfo = reactive({
    platCode: 'tiktok'
  });
  const initTplInfo = async () => {
    tplInfo.languageCode = null;
    tplInfo.templateTypeName = null;
    tplId.value = null;
    const { data } = await getTplTypeListApi('tiktok');
    tplTypeList.value = data;
  };
  // 获取模板类型
  const tplTypeList = ref([]);
  const handleGetTplNameList = async () => {
    tplNameList.value = null;
    tplId.value = null;
    if (tplInfo.languageCode && tplInfo.templateTypeName) {
      const { data } = await getTplNameListApi({ ...tplInfo });
      tplNameList.value = data;
    }
  };
  const tplNameList = ref([]);
  const tplId = ref();
  const handleTplName = async () => {
    const { data } = await getTplContentApi(tplId.value);
    textarea.value = data.emailContent;
  };
  // #endregion 语言模板

  // 获取
  const changeMessageAttr = ref(false);
  onBeforeUpdate(() => {
    if (!changeMessageAttr.value) {
      scrollToBottom();
    } else {
      changeMessageAttr.value = false;
    }
  });
  defineExpose({ scrollToBottom });
</script>

<style lang="scss" scoped>
  @import url('@/components/lazada/css/common.css');
  .tikTokChat {
    flex: 1;
  }
  .chat_header {
    display: flex;
    flex-direction: row;
    height: 80px !important;
    align-items: center;
    margin-left: 10px;
    &_info {
      display: flex;
      flex-direction: column;
      padding-left: 20px;
      justify-content: space-between;
      & span:first-child {
        font-size: 16px;
        font-weight: 600;
      }
      & span:last-child {
        font-size: 14px;
      }
      .shop-icon {
        color: --chat-primary;
        font-size: 18px;
        margin-top: 5px;
      }
    }
    &_store {
      margin-top: 10px;
      display: flex;
    }
    &_pinned {
      width: 100%;
      padding-right: 40px;
      text-align: right;
    }
  }

  .el-container.is-vertical {
    flex-direction: column;
  }

  .el-container {
    display: flex;
    flex-direction: row;
    flex: 1;
    flex-basis: auto;
    box-sizing: border-box;
    min-width: 0;
  }
  :deep(.el-textarea__inner) {
    :focus {
      border-color: #c0c4cc;
    }
  }
  .chat_header_info {
    width: 100%;
  }
  .tikTokChat {
    height: 100%;
    display: flex;
    flex-direction: column;
    .chat_main {
      display: block;
      flex: 1;
      box-sizing: border-box;
      padding: 20px;
      overflow: scroll;
      background-color: #f4f5f5;
    }
    .el-upload-list {
      display: none;
    }

    .border-bottom {
      border-bottom: 1px solid #ddd;
    }

    .chat-footer {
      height: 270px;
      border-top: 1px solid #ddd;
      .emoji-part {
        position: absolute;
        left: -140px;
        bottom: 50px;
        .emoji-scroll-content {
          width: 330px;
          height: 260px;
        }
      }
      .upload_img {
        vertical-align: middle;
        height: 100%;
      }
    }
    .m8 {
      margin: 8px;
    }
    .mtb8 {
      margin: 8px 0 8px 0;
    }
    .msg-status {
      display: flex;
      align-items: center;
      padding: 10px;
      font-size: 16px;
    }
  }
  .flex-center {
    display: flex;
    align-items: center;
  }
  .ml-8 {
    margin-left: 8px;
  }
  .collect_star {
    color: rgb(251, 191, 0);
  }
  .md_headpic {
    flex-shrink: 0;
  }
  .relative {
    position: relative;
  }
  .transform-box {
    z-index: 10;
    position: absolute;
    right: 15px;
    top: 75px;
  }
  .reply-box {
    z-index: 10;
    position: absolute;
    right: 15px;
    bottom: 10px;
  }
</style>
