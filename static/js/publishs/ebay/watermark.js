...
layui.use(['admin','table','form','layer','formSelects','laytpl', 'upload'],function(){
    var admin = layui.admin,
        ...
        form = layui.form;
    //渲染部门销售员店铺三级联动
    render_hp_orgs_users("#watermarkForm");
    //字体和字体大小数组集合
    ...
    var watermarkName = {
        trigClick: function(){
           $('[lay-filter=watermark_submit]').click();
        },
        ...
        newAdd: function(){ //新增水印
            var _this = this;
           $('#watermark_newAdd').on('click', function(){
               var watermarkData='';
               var index= layer.open({
                    ...
                    content: $('#watermark_newAddLayer').html(),
                    success: function(layero, index){
                        ...
                        formSelects.render();
                        form.on('submit(watermarkAdd_submit)', function(data){
                    ...
                    yes: function(index, layero){
                       ...
                       */
                        var $watermarkFontDiv = $('#watermarkFontDiv');
                            $('[lay-filter=watermarkAdd_submit]').trigger('click');
                            if(watermarkData.watermarkType ==1){ //表示文字水印
                                ...
                                //获取透明度和尺寸
                                watermarkData.watermarkTransparency = (100 -$('#watermarkTypeFont_opacityAmount').text().split('%')[0]);
                                watermarkData.watermarkSize = watermarkData.watermarkFontSize;
                                watermarkData.watermarkFontContent = $('#watermarkFontDiv').find('span').html();
                                //图片相关的都设为空
                                watermarkData.watermarkUrl = '';
                            }else if(watermarkData.watermarkType ==0){ //表示图片水印
                                //透明度
                                watermarkData.watermarkTransparency = (100 -$('#watermarkTypeImg_opacityAmount').text().split('%')[0]);
                                //文字相关的都设为空
                            ...
                            watermarkData.borderSize = $('[lay-filter=watermark_sizeChange]').val();
        ...
        toggleType: function(){ //监听radio的点击事件 然后决定显示什么内容
                //基础节点
                var $watermarkDiv = $('#watermarkImageDiv'),
                    $watermarkFontDiv = $('#watermarkFontDiv');
                form.on('radio(watermarkTypeRadio)', function(data){
                    if(data.value== 1){ //文字水印
                        /**文字水印设置显示,图片水印设置隐藏 */
                        $('#watermarkFontDiv').removeClass('disN');
                        $('.watermarkTypeImg').addClass('disN');
                        $('.watermarkTypeText').removeClass('disN');
                        $('#watermarkImageDiv').addClass('disN');
                        /**
                         * 切换后的图片水印设置初始化:
                         ...
                         * 3.图片透明度和尺寸进度条恢复默认
                         * */
                        $('#watermark_uploadImg').removeAttr('src');
                        ...
                        $('#watermarkTypeImg_opacityAmount').html('0%');
                        $('#slider-range-min1').slider({'value': 100});
                        $('#watermarkTypeImg_sizeAmount').html('100%');
                        $('#slider-range-min3').slider({'value': 0}); //文字透明度
                        $('#watermarkTypeFont_opacityAmount').html('0%');
                        $('#slider-range-min4').slider({'value': 0});
                        $('#watermarkTypeImg_rotateAmount').html(0);
                        $('#slider-range-min5').slider({'value': 0}); //文字旋转
                        $('#watermarkTypeFont_rotateAmount').html(0);
                        ...
                        /**文字水印设置隐藏,图片水印设置显示 */
                        $('#watermarkFontDiv').addClass('disN');
                        $('.watermarkTypeImg').removeClass('disN');
                        $('.watermarkTypeText').addClass('disN');
                        $('#watermarkImageDiv').removeClass('disN');
                        /**切换后的文字水印设置初始化*/
                        $('#slider-range-min2').slider({'value': 0});
                        $('#watermarkTypeImg_opacityAmount').html('0%'); //图片透明度
                        $('#slider-range-min1').slider({'value': 100});
                        $('#watermarkTypeImg_sizeAmount').html('100%'); //大小初始化
                        $('#slider-range-min3').slider({'value': 0});
                        $('#watermarkTypeFont_opacityAmount').html('0%'); //文字透明度
                        $('#slider-range-min4').slider({'value': 0});
                        $('#watermarkTypeImg_rotateAmount').html(0);
                        $('#slider-range-min5').slider({'value': 0}); //文字旋转
                        $('#watermarkTypeFont_rotateAmount').html(0);
        ...
        watermark: function(){ //功能集合函数
            ...
            var $watermarkDiv = $('#watermarkImageDiv'),
            $watermarkFontDiv = $('#watermarkFontDiv'),
            $sliderRangeMin1 = $('#slider-range-min1'),  //图片大小
            $sliderRangeMin2 = $('#slider-range-min2'), //透明度
            $sliderRangeMin3 = $('#slider-range-min3'),//文字透明度
            $sliderRangeMin4 = $('#slider-range-min4'),//图片旋转角度
            $sliderRangeMin5 = $('#slider-range-min5'),//文字旋转角度
            Width = $watermarkDiv.width(),
            Height = $watermarkDiv.height();
            //普通图片上传
            var uploadInst = upload.render({
                elem: '#watermark_upload'
                ,url: ctx + "/watermark/uploadPhoto.html?platCode=ebay"
                ,done: function(res){
                    if(res.code == '0000'){//上传成功
                        //重置图片和百分比
                        sliderScroll($sliderRangeMin1, 'watermarkTypeImg_sizeAmount', 100, 20, 200);
                        sliderScroll($sliderRangeMin2, 'watermarkTypeImg_opacityAmount', 0, 0, 100);
                        ...
                        $IMG.onload = function() { 
                            ...
                            $('#watermark_uploadImg').attr('src', res.data); //图片链接（base64）
            ...
            var sliderScroll = function(btn, id, value, min, max){
                btn.slider({
                    ...
                    slide: function(event, ui) {
                        if(id == 'watermarkTypeImg_rotateAmount'){
                            $('#'+id).html(ui.value);
                        }else{
                            $('#'+id).html(ui.value + '%');
                        }
                        if(id == 'watermarkTypeImg_sizeAmount'){
                        ...
                        }else if(id=='watermarkTypeImg_opacityAmount'){
            ...
            (function(){
                var mouseDownImg;
                $("#watermarkImageDiv > .watermark-div-fix").off("mousedown").on("mousedown", function() {
                ...
                mouseDownImg = drag($watermarkDiv, $('#albumListBody'));
            })();
            //水印文字拖拽
            (function(){
                var mouseDownFont;
                $("#watermarkFontDiv span").off("mousedown").on("mousedown", function() {
                    $(window).off("mousedown");
                    $watermarkFontDiv.on("mousedown", mouseDownFont);
                });
                mouseDownFont = drag($watermarkFontDiv, $('#albumListBody'));
            })();
            //文字颜色 背景和透明度的处理
            (function(){
                //水印文字输入的文字内容
                ...
                //监听选择的字体大小
                form.on('select(watermarkFontSizeF)', function(data){
                ...
                //监听字体类型
                form.on('select(watermarkFontTypeF)', function(data){
                ...
                $sliderRangeMin3.slider({
                    ...
                    max: 100, //最大值
                    slide: function(event, ui) {
                        $('#watermarkTypeFont_opacityAmount').html(ui.value + '%');
                        var opacity = ui.value/100;
                        $watermarkFontDiv.css({
                            'opacity': 1-opacity
                        });
                ...
                $('#watermarkTypeFont_opacityAmount').html($sliderRangeMin3.slider('value') + '%');
                /*监听文字背景透明度end*/
                //这是文字旋转度
                $sliderRangeMin5.slider({
                    ...
                    max: 180, //最大值
                    slide: function(event, ui) {
                        $('#watermarkTypeFont_rotateAmount').html(ui.value);
                        $watermarkFontDiv.css({
                            'transform': `rotate(${ui.value}deg)`
                        });
                        $('[name=degree]').val(ui.value);
            ...
            table.render({
                ...
                cols: [
                    [ //表头
                        ...
                        ,{title: '水印类型', templet: '#watermark_tableType'}
                        // ,{title: '水印类型', field: 'watermarkType'}
                        ,{title: '水印', templet: '#watermark_tableShow'}
                        // ,{title: '服务代码', templet: '#logisticsMode_tableCode'}
                        ,{title: '操作', align:'center', toolbar: '#watermark_tableIdBar'}
                   ]
                ],
                page: true,
                id: "watermark_tableId",
                limits: [50, 100, 300],
                limit: 50,
                done: function(res){                  
                    //工具条监听事件
        ...
        watchBar: function(){
            var _this = this;
            table.on('tool(watermark_tableFilter)',function(obj){
                var data = obj.data;
                // console.log(data);
        ...
        edit: function(data){ //水印模板编辑功能
            ...
            var index = layer.open({
                ...
                btn: ['保存', '关闭'],
                id: 'watermark_newAddLayerId',
                content: $('#watermark_editLayer').html(),
                success: function(layero, index){
                    var formTemplate = watermark_contentTpl.innerHTML;
                    var formDiv= document.getElementById('watermark_content');
                        ...
                        $('[lay-filter=watermarkAdd_submit]').trigger('click');
                          if(editWatermarkData.watermarkType ==1){ //表示文字水印
                            ...
                            //   console.log(coordinateX, coordinateY);
                              ...
                              //获取透明度和尺寸
                              editWatermarkData.watermarkTransparency = (100 -$('#watermarkTypeFont_opacityAmount').text().split('%')[0]);
                              editWatermarkData.watermarkSize = editWatermarkData.watermarkFontSize;
                              //图片相关的都设为空
                              editWatermarkData.watermarkUrl = '';
                              editWatermarkData.watermarkFontContent = $('#watermarkFontDiv').find('span').html();
                          }else if(editWatermarkData.watermarkType ==0){ //表示图片水印
                              //透明度
                              editWatermarkData.watermarkTransparency = (100 -$('#watermarkTypeImg_opacityAmount').text().split('%')[0]);
                              //文字相关的都设为空
                          ...
                          editWatermarkData.platCode = "ebay";
                          editWatermarkData.borderSize = $('[lay-filter=watermark_sizeChange]').val();
        ...
        watermarkEdit: function(data){
            ...
            //这个是图片的尺寸调整
            sliderScroll($sliderRangeMin1, 'watermarkTypeImg_sizeAmount', data.watermarkSize, 20, 200);
            //这个是透明度的调整
            sliderScroll($sliderRangeMin2, 'watermarkTypeImg_opacityAmount', (100-data.watermarkTransparency), 0, 100);
            //这个是图片的旋转角度
            sliderScroll($sliderRangeMin4, 'watermarkTypeImg_rotateAmount', data.degree,-180, 180);
            //拖拽功能
            var drag = function($watermarkDivs,$albumListBody){
                var watermarkType = $.trim($('[name=watermarkType]').val()); //获取水印类型
                //禁止选择网页中的文字
        ...
        watchSizeChange: function(){
            form.on('select(watermark_sizeChange)', function(obj){
                var val = obj.elem.value;
                if(val == 1000){
                    $('#watermark_rightBody').addClass('watermark-right-1000').removeClass('watermark-right-800');
                }else{
                    $('#watermark_rightBody').removeClass('watermark-right-1000').addClass('watermark-right-800');
                }
            })
        }
    };
    //监听表单提交事件
    form.on('submit(watermark_submit)', function(data){
        var data = data.field; //获取到表单提交对象
