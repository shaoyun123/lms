...
<script setup name="configurestoredarazstoreconf">
  import { reactive, ref, onMounted } from 'vue';
  import StoreConfig from '@/components/StoreConfig/index.vue';
  import { getSiteListApi } from '@/api/common/index.js';
  import { getCurrencyApi } from '@/api/configure/storeconf.js';

  const platCode = ref('daraz');
  ...
  const getCurrencyList = async () => {
    try {
      const { code, data } = await getCurrencyApi({ limit: 500 });
      if (code === '0000') {
        let currencyList = [];
        data?.forEach((item) => {
          let obj = {
            label: item.srcCyName,
            value: item.srcCyCode
          };
          currencyList.push(obj);
        });
        customFormConfig.forEach((item) => {
          if (item.contentField === 'currency') {
            item.contentOption = currencyList;
          }
        });
      }
    } catch (err) {
