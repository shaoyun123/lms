...
<script setup>
  ...
  const handleGross = async () => {
    ...
    const { data } = await listSelectedInfoApi({
      type: props.joinRightNowType,
      params
  ...
  const queryList = async () => {
    ...
    tableData.value = data.skus.map((item) => {
      // 可加入页签给原箱默认值; 已预约取接口返回的
      if (props.joinRightNowType === 1) {
        item.originalBox = 1;
      }
      ...
      // 特殊商品类型 初始渲染默认枚举
      if (item.specialProductTypes.length) {
  ...
  const handleSubmit = async () => {
    ...
    const { code, data, msg } = await batchSelectSubmitApi(params).finally(
