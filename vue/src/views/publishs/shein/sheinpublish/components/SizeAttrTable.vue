...
<script setup>
  ...
  const resetTableData = (type, val) => {
    ...
    if (val != '') {
      ...
      // 移除次规格
      changeSizeTableData('main', 'remove');
    }
  };
  ...
  const getAttrList = (option) => {
    ...
    if (option == 'remove') {
      ...
      if (subAttributeId.value != '') {
        arr = subArr.value;
      } else if (mainAttributeId.value != '') {
        arr = mainArr.value;
      }
    }
    return arr;
  };
  const changeSizeTableData = async (type = 'sub', option) => {
    // 如果数据为空，不进行操作
    if (!sheinSubSpecSalesAttrsAndSizeAttributeInfoDtos.value.length)
      return false;
    //
        ...
        if (!sheinPublishSkuDtosSubObj[item]) {
          sheinSubSpecSalesAttrsAndSizeAttributeInfoDtos.value =
            sheinSubSpecSalesAttrsAndSizeAttributeInfoDtos.value.filter(
              (elem) =>
                Object.keys(sheinPublishSkuDtosSubObj).includes(
                  elem.subSpecAttributeValueId
                )
            );
          // sheinSubSpecSalesAttrsAndSizeAttributeInfoDtos.value.filter(
